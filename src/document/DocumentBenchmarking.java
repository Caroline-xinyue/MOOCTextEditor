...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			long startTB = 0, endTB = 0, startTE = 0, endTE = 0;
			System.out.print(numToCheck + "\t");
			String text = getStringFromFile(textfile, numToCheck);
			startTB = System.nanoTime();
			for(int i = 0; i < trials; i++) {
				BasicDocument basicDocument = new BasicDocument(text);
				double fleshScoreB = basicDocument.getFleschScore();
			}
			endTB = System.nanoTime();
			System.out.print(endTB - startTB + "\t");
			// Efficient Document
			startTE = System.nanoTime();
			for(int i = 0; i < trials; i++) {
				EfficientDocument efficientDocument = new EfficientDocument(text);
				double fleshScoreE = efficientDocument.getFleschScore();
			}
			endTE = System.nanoTime();
			System.out.print(endTE - startTE + "\n");
