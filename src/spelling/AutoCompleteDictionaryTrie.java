...
public class AutoCompleteDictionaryTrie implements  Dictionary, AutoComplete {
     ...
     */@Override
     public List<String> predictCompletions(String prefix, int numCompletions) 
     {
    	 ...
    	 String toAdd = prefix.toLowerCase();
    	 TrieNode curr = root;
  		 Character c;
  		 for(int i = 0; i < toAdd.length(); i++) {
  			 c = toAdd.charAt(i);
    	 ...
    	 int count = 0;
    	 String word;
    	 trieQueue.offer(curr);
    	 while(!trieQueue.isEmpty() && count < numCompletions) {
    		 curr = trieQueue.poll();
    		 word = curr.getText();
    		 if(isWord(word)) {
    			 completion.add(word);
			 ...
			 for(Character ch : curr.getValidNextCharacters()){
				 trieQueue.offer(curr.getChild(ch));
			 }
    	 }
    	 return completion;
     }
