...
//			System.out.println(printQueue(queue));
			node = queue.remove(0);
			list = nw.distanceOne(node.getWord(), true);
//			System.out.println(list);
			for(String str : list) {
				if(!visited.contains(str)) {
					parent = node;
					children = new WPTreeNode(str, parent);
					queue.add(children);
//					System.out.println(printQueue(queue));
					visited.add(str);
					parent.addChild(str);
					if(str.equals(word2)) {
						return children.buildPathToRoot();
					}
				}
			}
		}
	...
	public static void main(String[] args) {
		WPTree testTree = new WPTree();
		List<String> result = testTree.findPath("time", "theme");
